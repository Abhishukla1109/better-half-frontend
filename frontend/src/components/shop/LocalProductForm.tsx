...
export default function LocalProductForm({ product }: Props) {
  ...
  if (!product.available) {
    return (
      <button disabled className="w-full py-4 rounded-xl bg-[#e5e7eb] text-[#9ca3af] text-base font-bold cursor-not-allowed">
        Out of Stock
      </button>
    );
  }
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <div className="flex items-center gap-4">
        <p className="text-sm font-semibold text-[#1a2e2e]">Qty</p>
        ...
        <span className="text-sm text-[#9ca3af]">₹{product.price.toLocaleString('en-IN')} each</span>
      </div>
